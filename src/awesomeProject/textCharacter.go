		...
		for scanner.Scan(){
			...
			if len(line)>0{
				...
				for _, sf := range subFields{
					...
					//if strings.HasPrefix(sf, "http"){
					//	r.urlStringToWord(sf)
					//}else{
					r.findWord(sf)
					//}
...
func (r *report) urlStringToWord(h string){
	urlStrct, err := url.Parse(h)
	if err != nil{
		log.Fatal(err)
	}

	r.addWord(urlStrct.Scheme, h )
	r.addWord(urlStrct.Opaque, h )
	r.addWord(urlStrct.Host, h)
	r.addWord(urlStrct.RawPath,h)
	r.addWord(urlStrct.RawQuery, h)
	r.addWord(urlStrct.Fragment, h)
